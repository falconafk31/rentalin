...
10. **BAST (`handovers`) — siklus hidup & snapshot historis (M4.1, migrasi 0027). Jangan dilanggar fitur berikutnya:**
    ...
    - **Duplikat BAST (M4.2):** pelanggaran unique `handovers_document_number_key` / `handovers_contract_type_unique` (23505) WAJIB diubah menjadi pesan bisnis Indonesia via `isUniqueViolation()` + `classifyBastUniqueError()` + `bastDuplicateMessage()` (`src/lib/bast.ts`) — **jangan** membocorkan teks mentah PostgreSQL. Constraint DB tetap otoritas final; pre-check `siblings` hanya UX, bukan mekanisme koreksi.
