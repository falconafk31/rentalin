...
export async function GET(request:Request,{params}:{params:Promise<{kind:string;id:string}>}){
 ...
 else if(handover){data.handover=true;const raw=handover as unknown as Record<string,boolean>;data.rows.push({label:'Jenis serah terima',value:labels[handover.type]});const items:[string,string][]=[['engine','Mesin'],['hydraulics','Sistem hidraulik'],['tracks','Rantai / roda'],['oil','Oli & cairan'],['fuel','Bahan bakar'],['battery','Aki & starter'],['lights','Lampu & klakson'],['brakes','Rem & kemudi'],['bucket','Bucket / attachment'],['cabin','Kabin & ROPS'],['safety','APAR & P3K'],['documents','SIKO & dokumen']];data.checklist=items.map(([key,item])=>({item,ok:!!raw[key]}));data.notes=handover.notes||'Para pihak telah memeriksa unit bersama-sama. Kondisi unit sesuai hasil pemeriksaan yang tercantum dalam berita acara ini.';}
