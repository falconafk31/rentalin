...
export async function GET(){
 ...
 const rows=[['LAPORAN OPERASIONAL',data.settings.companyName],['Tanggal',new Date().toLocaleDateString('id-ID')],[],['ARMADA'],['Kode Unit','Kategori','Merek / Model','Status','Lokasi','Tarif per Jam (Rp)'],...data.fleet.map(f=>[f.unitCode,f.category,f.brandModel,labels[f.status],f.currentLocation,f.hourlyRate]),[],['PENAGIHAN'],['Nomor Tagihan','Tanggal Terbit','Jatuh Tempo','Total (Rp)','PPN (Rp)','Dibayar (Rp)','Sisa (Rp)','Status'],...data.invoices.map(i=>{const paid=paidBy.get(i.id)||0;return [i.invoiceNumber,i.issueDate,i.dueDate,i.totalAmount,i.taxAmount,paid.toFixed(2),(Number(i.totalAmount)-paid).toFixed(2),labels[i.status]];})];
