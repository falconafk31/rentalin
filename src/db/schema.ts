...
}, (t) => [uniqueIndex('one_active_contract_per_unit').on(t.unitId).where(sql`${t.status} = 'active'`),check('contract_dates',sql`${t.endDate} >= ${t.startDate}`),check('contract_rate_positive',sql`${t.ratePerHour} > 0`),check('contract_operator_rate_type_valid',sql`${t.operatorRateType} IS NULL OR ${t.operatorRateType} IN ('hourly', 'daily')`),index('contracts_client_id_idx').on(t.clientId),index('contracts_status_idx').on(t.status),index('contracts_created_at_idx').on(t.createdAt)]);
...
}, (t) => [uniqueIndex('one_daily_log_per_contract').on(t.contractId,t.date),check('valid_meter_hours',sql`${t.startHm} >= 0 AND ${t.endHm} >= ${t.startHm} AND ${t.breakdownHours} >= 0 AND ${t.breakdownHours} <= ${t.endHm} - ${t.startHm}`),check('timesheets_operator_snapshot_consistent',sql`((operator_rate_snapshot IS NULL AND operator_rate_type_snapshot IS NULL) OR (operator_rate_snapshot IS NOT NULL AND operator_rate_type_snapshot IN ('hourly', 'daily')))`),index('timesheets_operator_id_idx').on(t.operatorId),index('timesheets_invoice_id_idx').on(t.invoiceId),index('timesheets_date_idx').on(t.date),index('timesheets_operator_driver_id_idx').on(t.operatorDriverId),index('timesheets_unit_id_idx').on(t.unitId)]);
export const handovers = pgTable('handovers', {
 id: uuid('id').defaultRandom().primaryKey(), documentNumber: text('document_number').notNull().unique(), contractId: uuid('contract_id').references(() => contracts.id).notNull(), type: text('type').notNull(), date: date('date').notNull(), engine: boolean('engine').notNull(), hydraulics: boolean('hydraulics').notNull(), tracks: boolean('tracks').notNull(), oil: boolean('oil').notNull().default(true), fuel: boolean('fuel').notNull().default(true), battery: boolean('battery').notNull().default(true), lights: boolean('lights').notNull().default(true), brakes: boolean('brakes').notNull().default(true), bucket: boolean('bucket').notNull().default(true), cabin: boolean('cabin').notNull().default(true), safety: boolean('safety').notNull().default(true), documents: boolean('documents').notNull().default(true), notes: text('notes'), photoUrls: text('photo_urls').array().notNull().default(sql`'{}'`),
 // M4.1 (migrasi 0027): siklus hidup draft -> final + snapshot historis.
 // contractId/type/documentNumber immutable setelah dibuat; setelah `final`
 // seluruh isi & snapshot beku (dijaga Server Action, bukan hanya UI).
 status: text('status').default('draft').notNull(),
 // Snapshot nullable = warisan pra-0027. FAIL-CLOSED: PDF TIDAK PERNAH
 // mengganti snapshot NULL dengan data live (pola M1.3 timesheets).
 clientNameSnapshot: text('client_name_snapshot'), unitCodeSnapshot: text('unit_code_snapshot'),
 unitModelSnapshot: text('unit_model_snapshot'), rateAtHandover: numeric('rate_at_handover', { precision: 12, scale: 2 }),
 createdAt: createdAt(),
}, (t) => [index('handovers_contract_id_idx').on(t.contractId),index('handovers_date_idx').on(t.date),uniqueIndex('handovers_contract_type_unique').on(t.contractId, t.type),index('handovers_status_idx').on(t.status),check('handovers_status_valid',sql`${t.status} IN ('draft', 'final')`),check('handovers_rate_at_handover_positive',sql`${t.rateAtHandover} IS NULL OR ${t.rateAtHandover} > 0`)]);
...
}, (t) => [check('payments_amount_positive', sql`${t.amount} > 0`), check('payments_method_valid', sql`${t.method} IN ('transfer', 'cash', 'giro', 'other')`), index('payments_invoice_id_idx').on(t.invoiceId)]);
...
}, (t) => [check('operators_rate_per_hour_non_negative', sql`${t.ratePerHour} >= 0`), check('operators_rate_per_day_non_negative', sql`${t.ratePerDay} >= 0`), check('operators_default_rate_type_valid', sql`${t.defaultRateType} IN ('hourly', 'daily')`), check('operators_status_valid', sql`${t.status} IN ('active', 'inactive')`), index('operators_status_idx').on(t.status), index('operators_name_idx').on(t.fullName), index('operators_sio_expiry_idx').on(t.sioExpiry)]);
