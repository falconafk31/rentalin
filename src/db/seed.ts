...
export const seedPreview = cache(async () => {
 ...
 await db.transaction(async tx => {
  ...
  const insertedInvoices = await tx.insert(s.invoices).values(totals.flatMap((total,i)=>Array.from({length:i===5?8:2},(_,j)=>{const d = new Date(now.getFullYear(),now.getMonth()-5+i,5+j*2); const due = new Date(d);due.setDate(due.getDate()+30);const amount=total/(i===5?8:2);const subtotal=Math.round(amount/1.11*100)/100;const tax=Math.round(subtotal*0.11*100)/100;return {invoiceNumber:`INV/${d.getFullYear()}/${String(d.getMonth()+1).padStart(2,'0')}/${String(j+1).padStart(3,'0')}`,contractId:agreements[j%agreements.length].id,subtotalAmount:subtotal.toFixed(2),totalAmount:(subtotal+tax).toFixed(2),taxAmount:tax.toFixed(2),taxRate:'11',status:i===5?(j===0?'overdue':'unpaid'):'paid',issueDate:d.toISOString().slice(0,10),dueDate:due.toISOString().slice(0,10)};}))).returning();
  ...
  if (paidInvoices.length) {
    await tx.insert(s.payments).values(
      paidInvoices.map(inv => ({
        ...
        method: 'transfer',
        reference: `TRF/${inv.invoiceNumber.replace(/\//g, '-')}`,
        notes: 'Pelunasan invoice via transfer bank (data demo).',
        paidAt: inv.dueDate,
        notedBy: demoId,
      }))
    );
