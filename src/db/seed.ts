...
export async function seedPreview() {
 ...
 await db.transaction(async tx => {
  ...
  await tx.insert(s.invoices).values(totals.flatMap((total,i)=>Array.from({length:i===5?8:2},(_,j)=>{const d = new Date(now.getFullYear(),now.getMonth()-5+i,5+j*2); const due = new Date(d);due.setDate(due.getDate()+30);const amount=total/(i===5?8:2);return {invoiceNumber:`INV/${d.getFullYear()}/${String(d.getMonth()+1).padStart(2,'0')}/${String(j+1).padStart(3,'0')}`,contractId:agreements[j%agreements.length].id,totalAmount:amount.toFixed(2),taxAmount:(amount*11/111).toFixed(2),status:i===5?(j===0?'overdue':'unpaid'):'paid',issueDate:d.toISOString().slice(0,10),dueDate:due.toISOString().slice(0,10)};})));
