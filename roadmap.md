...
| 2.24 | **M4.2: hardening BAST — duplikat, otorisasi, foto, uji** | (F5) pelanggaran unique BAST (23505) diubah menjadi pesan bisnis Indonesia yang aman — constraint `handovers_document_number_key` / `handovers_contract_type_unique` tetap otoritas final, pre-check hanya UX; (F6) GET/POST `bast-photos` memakai daftar role eksplisit (tanpa allow-all), tolak path traversal & null-byte sebelum query DB, foto di-resolve lewat `photo_urls` terdaftar + `isUserAuthorizedForBastPhoto` per-entitas (operator hanya kontrak tertugas), signed URL 1 jam tetap satu-satunya jalur baca; (F8) `scripts/bast-check.ts` diperluas ke 23 assertion uji (§14–16: klasifikasi 23505, invarian sumber otorisasi/foto, matriks peran) di samping snapshot/lifecycle/lock-order M4.1 yang tetap hijau; (F3 penomoran BAST: **ditunda eksplisit** — penomoran memakai `pg_advisory_xact_lock` per prefix+tahun yang sudah atomik, tidak ada MAX()+1; tidak ada arsitektur penomoran baru) — selesai (kode, PR menyusul) |
